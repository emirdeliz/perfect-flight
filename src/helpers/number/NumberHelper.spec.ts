...
describe('helpers/number/formatNumberAsCurrency', () => {
  test('Should have format number 1', () => {
    expect(formatNumberAsCurrency(1).replace(/\s/g, '')).toBe('$1.00');
  });
  ...
  test('Should have format number undefined', () => {
    expect(formatNumberAsCurrency(undefined).replace(/\s/g, '')).toBe('$0.00');
  });

  test('Should have format number Infinity', () => {
    expect(formatNumberAsCurrency(Infinity).replace(/\s/g, '')).toBe('$0.00');
  });
  ...
  test('Should have format number NaN', () => {
    expect(formatNumberAsCurrency(NaN).replace(/\s/g, '')).toBe('$0.00');
  });
